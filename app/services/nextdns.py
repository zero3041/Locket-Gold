...
async def create_profile(api_key, log_callback=None, profile_name=None):
    def log(msg):
        if log_callback:
            log_callback(msg)
    ...
    # A fixed profile_name (e.g. "LocketVIP-Permanent") is reused forever and shared
    # by everyone; when omitted we fall back to the per-day name used by the Gold flow.
    if not profile_name:
        ...
        try:
            async with session.post(create_url, json=payload) as response:
                if response.status == 200:
                    ...
                    log(f"[>] Enabling logging (usage tracking)...")
                    try:
                        async with session.patch(
                            f"https://api.nextdns.io/profiles/{pid}/settings/logs",
                            json={"enabled": True}
                        ) as lg:
                            pass
                    except Exception:
